...
class TaboolaCampaignRequest extends BaseRequest {
    ...
    /**
     * Mass upate, $data['collection'=>[..items..]
     *
     * @param string $account_id
     * @param string $campaign_id
     * @param array $data
     * @return \TaboolaApi\Client\Responses\Response
     */
    public function createCampaignItems(string $account_id, string $campaign_id, array $data)
    {
        if (!array_key_exists('collection', $data)) {
            $data = ['collection'=>$data];
        }
        return $this->request('post', $data, sprintf($this->endpoint . DS . '%s/items/mass', $account_id, $campaign_id));
    }
